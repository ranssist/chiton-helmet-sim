...
| A-37 | 메시 머리둘레는 단면의 **볼록껍질** 둘레로 잰다 | 닫힌 셸을 자르면 안팎 루프가 같이 나와 합이 2~3배가 된다. 볼록껍질 = 바깥 윤곽 |
| A-38 | 삼각형 20만 개 초과면 투영면적을 격자로 근사 | 합집합이 끝나지 않는다. 검증: 구 81,920면에서 합집합 대비 +0.07 % |
...
10단계 모두 구현했고, 오차 감축 기능(판 강성 실측·막 강성·예측 밴드)과 UX 개선까지 넣어 `pytest` 131건이 통과한다(약 80초). 단계별 테스트 수는 다음과 같다.
...
| 14 | 3D 모델 (mesh) | 10 | 축척 없으면 면적을 `미확인`으로 막음, 머리둘레+라이너·셸 보정, 안/바깥 면 한 쌍 판별, 투영면적 = 삼각형 합집합 |
