...
대용량·닫힌 모델도 읽는다. 삼각형이 20만 개를 넘으면 투영 면적을 격자로 근사하고(구 81,920면 검증에서
합집합 대비 +0.07 %), 머리둘레는 단면의 볼록껍질 둘레 = 바깥 윤곽으로 잰다(닫힌 셸은 안팎 루프가 같이
잡혀 그냥 합하면 2~3배가 된다). 527만 면 STL 기준 약 1분 걸린다.
